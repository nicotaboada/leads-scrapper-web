...
export function CreateRunSheet({
	...
	const form = useForm<CreateRunFormInput>({
		...
		defaultValues: {
			...
			scrapeLeads: false,
			maxLeadsPerPlace: undefined,
		},
		mode: 'onBlur',
		reValidateMode: 'onChange',
	})

	// Watch actor type and scrapeLeads to show/hide configuration sections
	const selectedActorType = form.watch('actorType')
	const scrapeLeadsEnabled = form.watch('scrapeLeads')
	...
	async function handleSubmit(data: CreateRunFormInput) {
		try {
			...
			// Map form field names to Apify API parameter names
			const input: CreateRunInput = {
				...
				input: {
					...
					// Apify uses 'scrapeReviewsPersonalData' for leads enrichment
					scrapeReviewsPersonalData: data.scrapeLeads ?? false,
					// When leads enrichment is enabled, add required parameters
					...(data.scrapeLeads && {
						// Hardcoded departments we want to scrape
						leadsEnrichmentDepartments: ['operations', 'marketing', 'sales'],
						// Maximum leads per place (Apify parameter name)
						...(data.maxLeadsPerPlace && {
							maximumLeadsEnrichmentRecords: data.maxLeadsPerPlace,
						}),
					}),
				},
			}
