...
const LINK_CLASSES =
	'font-medium text-foreground hover:underline decoration-muted-foreground/50 underline-offset-4 transition-colors'
...
function ActivityMessage({ activity }: { activity: UserActivity }) {
	...
	switch (type) {
		...
		case UserActivityType.RUN_CREATED:
			return (
				<span className="text-muted-foreground">
					...
					{metadata.runId ? (
						<Link href={`/runs/${metadata.runId}`} className={LINK_CLASSES}>
							{metadata.runName}
						</Link>
					) : (
						<span className="text-foreground font-medium">
		...
		default:
			return (
				<span className="text-muted-foreground">ha realizado una acción</span>
			)
	}
}
			...
			<div className="relative flex-shrink-0">
				...
				<div className="absolute -right-1 -bottom-1">
					<ActivityIcon
						type={activity.type}
						size={compact ? 'xs' : 'sm'}
						className="border-background border-2 shadow-sm"
					/>
				</div>
			</div>
